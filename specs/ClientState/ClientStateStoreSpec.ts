...
describe('ClientState.ClientStateStore', () => {
	...
	it('should create clientState by CREATE_IF_NOT_EXISTS action', (done: Function) => {
		dispatcher.dispatch(clientStateActionCreator.createIfNotExists('myClientId'));
		// wait 1 tick
		setTimeout(() => {
	...
	it('should update all clientStates by UPDATE action & emit SEND_DIFF actions for every clientState', (done: Function) => {
		var countOfCallbackExecuted = 0;
		dispatcher.dispatch(clientStateActionCreator.createIfNotExists('myClientId'));
		dispatcher.dispatch(clientStateActionCreator.createIfNotExists('yourClientId'));
		// wait 1 tick
		setTimeout(() => {
			...
			dispatcher.dispatch(clientStateActionCreator.update(updateCallback));
	...
	it('should update specific clientState by UPDATE_CLIENT action & emit SEND_DIFF actions for every clientState', (done: Function) => {
		...
		// wait 1 tick
		setTimeout(() => {
			dispatcher.dispatch(clientStateActionCreator.updateClient('myClientId', (clientState: IClientState, clientId: string) => {
				clientState = clientState.set('storedClientId', clientId);
				clientState = clientState.set('specific', 'my');
				return clientState;
			}));
			dispatcher.dispatch(clientStateActionCreator.updateClient('yourClientId', (clientState: IClientState, clientId: string) => {
				clientState = clientState.set('storedClientId', clientId);
	...
	it('should emit SEND_DIFF action after every UPDATE_CLIENT action with OPS changes using RFC 6902', (done: Function) => {
		...
		setTimeout(() => {
			...
			dispatcher.dispatch(clientStateActionCreator.updateClient('myClientId', (clientState: IClientState) => {
